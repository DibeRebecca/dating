    ...
    try {
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        ...
        sharedPreferences.setString(
            "user_data", jsonEncode(response.data["body"]["user"]));
        sharedPreferences.setString(
            "token", response.data["body"]["accessToken"]);
        sharedPreferences.setString(
            "refresh_token", response.data["body"]["refreshToken"]);
        sharedPreferences.setString("userphone", data["phone"]["e164"]);
        sharedPreferences.setString("user_password", data["password"]);

        return SuccessLogin(code: 200, response: "compte créé avec succès", data: UserLoginResponse.fromMap(response.data["body"]["user"],) );
      } else {
        return ErrorLogin(
            code: 500,
